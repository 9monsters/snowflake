...
func TestSnowflakeOnce(t *testing.T) {
	sleepTime := time.Duration(50 * snowflakeTimeUnit)
	time.Sleep(sleepTime)

	id := nextID(t)

	actualTime := ElapsedTime(id)
	if actualTime < sleepTime || actualTime > sleepTime+snowflakeTimeUnit {
...
func TestSnowflakeTimeUnit(t *testing.T) {
	if time.Duration(snowflakeTimeUnit) != 10*time.Millisecond {
